...
//#define h 1
//#define coeff 100
//#define tauExplicit 0.01
//#define tauImplicit 0.4
//#define  tol    1.0e-6
//#define  omega_c 1.12
//
...
//
////======================================
//__global__ void presol_kernel(double *dev_u0, double *dev_uq)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//	dev_uq[offset] = (1 - 4 * (tauExplicit / (h*h)))*dev_u0[offset] + (tauExplicit / (h*h))*(dev_u0[top] + dev_u0[bottom] + dev_u0[left] + dev_u0[right]);
//}
//
//__device__ double gradient(double ux, double uy)
//{
...
//
//
//__global__ void pm_kernel(double *dev_u, double *dev_u0, double *dev_uq, double *dev_ae, double *dev_an, double *dev_aw, double *dev_as)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//	//Gradient variables
//	double uqx = 0.0, uqy = 0.0, sor;
//	//ae
//	uqx = (dev_uq[east] - dev_uq[offset]) / h;
//	uqy = (dev_uq[north] + dev_uq[neast] - dev_uq[south] - dev_uq[seast]) / (4 * h);
//	dev_ae[offset] = gradient(uqx, uqy);
//	//an
//	uqx = (dev_uq[north] - dev_uq[offset]) / h;
//	uqy = (dev_uq[nwest] + dev_uq[west] - dev_uq[neast] - dev_uq[east]) / (4 * h);
//	dev_an[offset] = gradient(uqx, uqy);
//	//aw
//	uqx = (dev_uq[west] - dev_uq[offset]) / h;
//	uqy = (dev_uq[nwest] + dev_uq[north] - dev_uq[south] - dev_uq[swest]) / (4 * h);
//	dev_aw[offset] = gradient(uqx, uqy);
//	//as
//	uqx = (dev_uq[south] - dev_uq[offset]) / h;
//	uqy = (dev_uq[west] + dev_uq[swest] - dev_uq[east] - dev_uq[seast]) / (4 * h);
//	dev_as[offset] = gradient(uqx, uqy);
//
//	//Evaluate the new U
//	// Initialize using SOR method
//	sor = (dev_u0[offset] + (tauImplicit / (h*h))*(dev_as[offset] * dev_uq[south] + dev_an[offset] * dev_uq[north] + dev_ae[offset] * dev_uq[east] + dev_aw[offset] * dev_uq[west])) / (1 + (tauImplicit / (h*h))*(dev_ae[offset] + dev_an[offset] + dev_as[offset] + dev_aw[offset]));
//	dev_u[offset] = dev_uq[offset] + omega_c*(sor - dev_uq[offset]);
//
//}
//
//
//__global__ void error_kernel(double *error, double *dev_u0, double *dev_u, double *dev_ae, double *dev_an, double *dev_aw, double *dev_as)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//
//	error[offset] = pow(dev_u[offset] * (1 + ((tauImplicit/(h*h))*(dev_ae[offset] + dev_an[offset] + dev_as[offset] + dev_aw[offset]))) - (tauImplicit / (h*h))*(dev_an[offset]*dev_u[top] + dev_as[offset]*dev_u[bottom] + dev_aw[offset]*dev_u[left] + dev_ae[offset]*dev_u[right]) - dev_u0[offset], 2);
//}
//
//// Main Function
//
...
//
//	/*GPU variables*/
//	double *d_u0, *d_u, *d_uq, *d_error, *d_ae, *d_an, *d_aw, *d_as;
...
//
//	int z; //Tolerance steps control variable 
//	int ni = 1000; //Steps
//	double error;
//	
//
...
//		presol_kernel << <blocks, threads >> > (d_u0, d_uq);
//		// Copy back uq to Host
//		HANDLE_ERROR(cudaMemcpy(uq, d_uq, img_size * sizeof(double), cudaMemcpyDeviceToHost));
//		//Perform Reflection of uq
//		reflexia(uq);
//		//Copy back uq to Device
//		HANDLE_ERROR(cudaMemcpy(d_uq, uq, img_size * sizeof(double), cudaMemcpyHostToDevice));
//		//Evaluate PM-Implicit
//		z = 0;
//		do 
//		{
//			z = z + 1;
//
//			pm_kernel << <blocks, threads >> > (d_u, d_u0, d_uq, d_ae, d_an, d_aw, d_as);
//
//			// Evaluate error
...
//				}
//			
//			//Copy error to Device from Host
//			HANDLE_ERROR(cudaMemcpy(d_error, partial_error, img_size * sizeof(double), cudaMemcpyHostToDevice));
//			
//			// Evaluate error
//			error_kernel << <blocks, threads >> > (d_error, d_u0, d_u, d_ae, d_an, d_aw, d_as);
//			
//			// Copy partial_error from device back to Host
//			HANDLE_ERROR(cudaMemcpy(partial_error, d_error, img_size * sizeof(double), cudaMemcpyDeviceToHost));
//			
//			for (k = 0; k < img_size; k++)
//				{
//					error += partial_error[k];
//				}
//
...
//			sprintf(name_ending, "_%d_pm.pgm", i);
//			strcat(name, name_ending);
//			strcpy(file_path, "images/modified/pm/implicit/");
