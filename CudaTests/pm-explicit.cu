...
__global__ void presol_kernel(double *dev_u0, double *dev_uq)
{
	...
	dev_uq[offset] = (1 - 4*(tau / (h*h)))*dev_u0[offset] + (tau / (h*h))*(dev_u0[top] + dev_u0[bottom] + dev_u0[left] + dev_u0[right]);
}
...
__global__ void pm_kernel(double *dev_u, double *dev_uq) 
{
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
...
int main()
{
	...
	*/
	...
	printf("AOD of the picture before is: %lf \n", tmp[0] / ((double)(n1 + 1)*(n2 + 1)));
	...
	for (i = 1; i <= kk; i++)
	{
		...
		////Evaluate PM
		//pm_kernel << <blocks, threads >> > (d_u, d_uq);
		...
		if ((i%iz) == 0)
		{
			...
			strcat(name, name_ending);
			strcpy(file_path, "images/modified/pm/fd/");
	...
	printf("AOD of the picture after is: %lf \n", tmp[0] / ((double)(n1 + 1)*(n2 + 1)));
