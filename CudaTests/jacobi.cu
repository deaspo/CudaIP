...
//
////======================================
//__global__ void heat_jacobi(double *dev_u0, double *dev_u, double tau, int h)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//	dev_u[offset] = (dev_u0[offset] + (tau / (h*h))*(dev_u[top] + dev_u[bottom] + dev_u[left] + dev_u[right]))/(1 + 4.0 * (tau / (h*h)));
//}
//
//
//__global__ void error_kernel(double *error, double *dev_u0, double *dev_u, double tau, int h)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//		{
//			z = z + 1;
//			heat_jacobi << <blocks, threads >> > (d_u0, d_u, tau, h);
//
//			// Evaluate error
...
//
//			// Evaluate error
//			error_kernel << <blocks, threads >> > (d_error, d_u0, d_u, tau, h);
//
...
//			sprintf(name_ending, "_jacobi_%d_lhe.pgm", i);
//			strcat(name, name_ending);
//			strcpy(file_path, "images/modified/implicit/jacobi/");
