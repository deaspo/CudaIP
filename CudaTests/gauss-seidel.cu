...
//#define  p     1
//#define  tol    1.0e-6   /* tolerance for accuracy gauss-seidel */
//#define  omega_c 1.12
//
...
//
//
//void write_new_image_data(pole1 u)
//{
//	int i, j;
...
//
////------------------------------------------------------------------------------------
//// ToDo To create kernels for this
...
////==============================================================================
//// ToDo To create kernels for this
//__global__ void min_max_kernel(double *dev_u, double *dev_u0, unsigned int *d_min, unsigned int *d_max)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//	dev_u0[offset] = sqrt(pow((dev_u[top] + dev_u[bottom]) / h, 2) + pow((dev_u[left] + dev_u[right]) / h, 2));
//	if (dev_u0[offset] < d_min[0]) d_min[0] = dev_u0[offset];
//	if (dev_u0[offset] > d_max[0]) d_max[0] = dev_u0[offset];
//}
//
//__global__ void fshs_kernel(double *dev_u0, unsigned int *d_min, unsigned int *d_max)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//	double P, L;
//	P = 1.0 / (double)(d_max[0] - d_min[0]);
//	L = -d_min[0] *P;
//
//	dev_u0[offset] = (P*dev_u0[offset] + L);
//	
//}
//
//void edge_detect(pole1 u)
//{
//	int i, j;
//	double min = 100000, max = 0, P, L;
//
//	for (i = p; i <= n1 + p; i++)
//	{
//		for (j = p; j <= n2 + p; j++)
//		{
//			u0[i][j] = sqrt(pow((u[i][j - 1] - u[i][j + 1]) / h, 2) + pow((u[i - 1][j] - u[i + 1][j]) / h, 2));
//			if (u0[i][j]<min) min = u0[i][j];
//			if (u0[i][j]>max) max = u0[i][j];
//		}
//	}
//	P = 1.0 / (double)(max - min);
//	L = -min*P;
//	for (i = p; i <= n1 + p; i++)
//	{
//		for (j = p; j <= n2 + p; j++)
...
////Kernel functions
//
//__global__ void gauss_seidel(double *dev_u0, double *dev_u, double tau)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//	jacob = (dev_u0[offset] + (tau / (h*h))*(dev_u[top] + dev_u[bottom] + dev_u[left] + dev_u[right])) / (1 + 4.0 * (tau / (h*h)));
//
//	dev_u[offset] = dev_u[offset] + omega_c*(jacob - dev_u[offset]);
//}
//
//
//__global__ void error_kernel(double *error, double *dev_u0, double *dev_u, double tau)
//{
//	int x = threadIdx.x + blockIdx.x * blockDim.x;
//	int y = threadIdx.y + blockIdx.y * blockDim.y;
//
...
//
//
//	error[offset] = pow(dev_u[offset] * (1 + 4.0*(tau / (h*h))) - (tau / (h*h))*(dev_u[top] + dev_u[bottom] + dev_u[left] + dev_u[right]) - dev_u0[offset], 2);
//}
//
//
//// Main Function
...
//
//	/*GPU variables*/
//	double *d_u0, *d_u, *d_error;
//	unsigned int *d_v, *d_tmp;
//	unsigned int *d_max, *d_min;
//
//	/* Important!
//	* Dim % Sqrt(threads) == 0
...
//	//
//	HANDLE_ERROR(cudaMalloc((void **)&d_max, 1 * sizeof(long)));
//	HANDLE_ERROR(cudaMemset(d_tmp, 0, 1 * sizeof(int)));
//
//	HANDLE_ERROR(cudaMalloc((void **)&d_min, 1 * sizeof(long)));
//	HANDLE_ERROR(cudaMemset(d_min, 100000, 1 * sizeof(int)));
//
...
//
//	/*Perform Heat Gauss-Seidel*/
//	double tau = 0.3, error;
//	partial_error = (double *)malloc(sizeof(double)*img_size);
//
//	int z;
//	int ni = 100;
//	reflexia(u);
//	// Copy u to d_u -  only once before execution
...
//
//		// Perform heat implicit Jacobi
//		z = 0;
//		do
//		{
//			z = z + 1;
//			gauss_seidel << <blocks, threads >> > (d_u0, d_u, tau);
//
//			// Evaluate error
...
//
//			// Evaluate error
//			error_kernel << <blocks, threads >> > (d_error, d_u0, d_u, tau);
//
...
//			sprintf(name_ending, "_%d_gauss.pgm", i);
//			strcat(name, name_ending);
//			strcpy(file_path, "images/modified/implicit/gauss/");
//			write_new_image_data(u);
//
//			// Apply Edge Detect
//			edge_detect(u);
//
//			strcpy(name, (*image).name);
//			sprintf(name_ending, "_%d_edge.pgm", i);
//			strcat(name, name_ending);
//			strcpy(file_path, "images/modified/implicit/gauss/edge/");
//			write_new_image_data(u0);
//		}
//
//	}
//	// AOD - function
